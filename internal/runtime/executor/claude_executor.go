...
func (e *ClaudeExecutor) HttpRequest(ctx context.Context, auth *cliproxyauth.Auth, req *http.Request) (*http.Response, error) {
	...
	resp, _, err := e.executeWithOAuth401Retry(ctx, auth, func(currentAuth *cliproxyauth.Auth) (*http.Response, error) {
		attemptReq := cloneClaudeHTTPRequest(ctx, req, reqBody)
		if errPrep := e.PrepareRequest(attemptReq, currentAuth); errPrep != nil {
			return nil, errPrep
		}
		httpClient := helps.NewUtlsHTTPClient(ctx, e.cfg, currentAuth, 0)
...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	ctx, err = withResolvedClaudeRequestContext(ctx, e.cfg, auth, apiKey)
	if err != nil {
		return resp, err
	}
	...
	restoreMap := map[string]string(nil)
	if rules := resolveClaudeToolNameTransformations(e.cfg, auth, apiKey); len(rules) > 0 {
		bodyForUpstream, restoreMap = applyClaudeToolNameTransformations(bodyForUpstream, rules)
	}
	if oauthToken {
	...
	httpResp, auth, err := e.executeWithOAuth401Retry(ctx, auth, func(currentAuth *cliproxyauth.Auth) (*http.Response, error) {
		...
		attemptCtx, errCtx := withResolvedClaudeRequestContext(ctx, e.cfg, currentAuth, currentAPIKey)
		if errCtx != nil {
			return nil, errCtx
		}
		...
		if errHeaders := applyClaudeHeaders(httpReq, baseModel, currentAuth, currentAPIKey, false, extraBetas, e.cfg); errHeaders != nil {
	...
	if len(restoreMap) > 0 {
		data = restoreClaudeToolNamesInResponse(data, restoreMap)
	}
	var param any
...
func (e *ClaudeExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	httpResp, auth, err := e.executeWithOAuth401Retry(ctx, auth, func(currentAuth *cliproxyauth.Auth) (*http.Response, error) {
		...
		if errHeaders := applyClaudeHeaders(httpReq, baseModel, currentAuth, currentAPIKey, true, extraBetas, e.cfg); errHeaders != nil {
			return nil, errHeaders
		}
		var authID, authLabel, authType, authValue string
		if currentAuth != nil {
	...
	go func() {
		...
		if responseFormat == to {
			...
			for scanner.Scan() {
				...
				if len(restoreMap) > 0 {
					line = restoreClaudeToolNamesInStreamLine(line, restoreMap)
				}
				event.Write(line)
				event.WriteByte('\n')
...
func (e *ClaudeExecutor) CountTokens(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	extraBetas, body = extractAndRemoveBetas(body)
	if rules := resolveClaudeToolNameTransformations(e.cfg, auth, apiKey); len(rules) > 0 {
		body, _ = applyClaudeToolNameTransformations(body, rules)
	}
...
func applyClaudeHeaders(r *http.Request, baseModel string, auth *cliproxyauth.Auth, apiKey string, stream bool, extraBetas []string, cfg *config.Config) error {
	if r == nil {
		return nil
	}
	...
	// Values below match Claude Code 2.1.76 / @anthropic-ai/sdk 0.74.0 (updated 2026-02-28).
	...
	if sessionID == "" {
		...
		sessionID, errSessionID = resolveClaudeSessionIDFromHeaders(r.Context(), ginHeaders, apiKey)
		if errSessionID != nil {
			return errSessionID
		}
	}
...
func resolveClaudeToolNameTransformations(cfg *config.Config, auth *cliproxyauth.Auth, apiKey string) []config.ClaudeToolNameTransformation {
	if cfg == nil || !isClaudeOAuthToken(apiKey) {
		return nil
	}
	if auth != nil && auth.ToolPrefixDisabled() {
		return nil
	}
...
func applyClaudeToolNameTransformations(body []byte, rules []config.ClaudeToolNameTransformation) ([]byte, map[string]string) {
	...
	recordTransformation := func(original, transformed string) {
		if original == "" || transformed == "" || original == transformed {
			return
		}
		if _, exists := restoreMap[transformed]; !exists {
			restoreMap[transformed] = original
		}
	}
	...
	if tools := gjson.GetBytes(body, "tools"); tools.Exists() && tools.IsArray() {
		tools.ForEach(func(index, tool gjson.Result) bool {
			...
			name := tool.Get("name").String()
			transformed, original, changed := transformClaudeToolName(name, rules, builtinTools)
			if !changed {
				return true
			}
			path := fmt.Sprintf("tools.%d.name", index.Int())
			body, _ = sjson.SetBytes(body, path, transformed)
			recordTransformation(original, transformed)
			return true
		})
	}
	...
	if gjson.GetBytes(body, "tool_choice.type").String() == "tool" {
		...
		transformed, original, changed := transformClaudeToolName(name, rules, builtinTools)
		if changed {
			body, _ = sjson.SetBytes(body, "tool_choice.name", transformed)
			recordTransformation(original, transformed)
		}
	}
	...
	if messages := gjson.GetBytes(body, "messages"); messages.Exists() && messages.IsArray() {
		messages.ForEach(func(msgIndex, msg gjson.Result) bool {
			...
			content.ForEach(func(contentIndex, part gjson.Result) bool {
				...
				case "tool_reference":
					toolName := part.Get("tool_name").String()
					transformed, original, changed := transformClaudeToolName(toolName, rules, builtinTools)
					if !changed {
						return true
					}
					path := fmt.Sprintf("messages.%d.content.%d.tool_name", msgIndex.Int(), contentIndex.Int())
				...
				case "tool_result":
					...
					if nestedContent.Exists() && nestedContent.IsArray() {
						nestedContent.ForEach(func(nestedIndex, nestedPart gjson.Result) bool {
							...
							nestedToolName := nestedPart.Get("tool_name").String()
							transformed, original, changed := transformClaudeToolName(nestedToolName, rules, builtinTools)
							if !changed {
								return true
							}
							nestedPath := fmt.Sprintf("messages.%d.content.%d.content.%d.tool_name", msgIndex.Int(), contentIndex.Int(), nestedIndex.Int())
							body, _ = sjson.SetBytes(body, nestedPath, transformed)
							recordTransformation(original, transformed)
							return true
						})
					}
	...
	return body, restoreMap
}

func transformClaudeToolName(name string, rules []config.ClaudeToolNameTransformation, builtinTools map[string]bool) (string, string, bool) {
	if name == "" || len(rules) == 0 {
		return name, "", false
	}
	if builtinTools != nil && builtinTools[name] {
		return name, "", false
	}
	...
	for _, rule := range rules {
		...
		prefix := fmt.Sprintf("mcp__%s__", server)
		if strings.HasPrefix(name, prefix) {
			original := strings.TrimPrefix(name, prefix)
			if original != "" && rule.Match(original) {
				return name, "", false
			}
		}
		...
		transformed := prefix + name
		if transformed == name {
			return name, "", false
		}
		return transformed, name, true
	}

	return name, "", false
}

func restoreClaudeToolNamesInResponse(body []byte, restoreMap map[string]string) []byte {
	if len(restoreMap) == 0 {
		return body
	}
	content := gjson.GetBytes(body, "content")
	...
	content.ForEach(func(index, part gjson.Result) bool {
		...
		case "tool_reference":
			toolName := part.Get("tool_name").String()
			original, ok := restoreMap[toolName]
			if !ok || original == "" {
				return true
			}
			path := fmt.Sprintf("content.%d.tool_name", index.Int())
		...
		case "tool_result":
			...
			if nestedContent.Exists() && nestedContent.IsArray() {
				nestedContent.ForEach(func(nestedIndex, nestedPart gjson.Result) bool {
					if nestedPart.Get("type").String() == "tool_reference" {
						nestedToolName := nestedPart.Get("tool_name").String()
						if original, ok := restoreMap[nestedToolName]; ok && original != "" {
...
func restoreClaudeToolNamesInStreamLine(line []byte, restoreMap map[string]string) []byte {
	if len(restoreMap) == 0 {
		return line
	}
	payload := helps.JSONPayload(line)
...
func resolveClaudeSessionIDFromHeaders(ctx context.Context, headers http.Header, apiKey string) (string, error) {
	if headers != nil {
...
func withResolvedClaudeRequestContext(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, apiKey string) (context.Context, error) {
	if ctx == nil {
		ctx = context.Background()
	}

	headers := getClientHeadersFromContext(ctx)
	sessionID, errSessionID := resolveClaudeSessionIDFromHeaders(ctx, headers, apiKey)
	if errSessionID != nil {
...
func injectClaudeUserID(ctx context.Context, payload []byte, cfg *config.Config, auth *cliproxyauth.Auth, apiKey string, useCache bool, effectiveVersion string, sessionID string) ([]byte, error) {
	...
	if system.IsArray() {
		system.ForEach(func(_, part gjson.Result) bool {
			...
			if !part.Get("cache_control").Exists() {
				partJSON = buildTextBlock(part.Get("text").String(), map[string]string{})
			}
			systemBlocks = append(systemBlocks, partJSON)
			return true
		})
	} else if system.Type == gjson.String && system.String() != "" {
		systemBlocks = append(systemBlocks, buildTextBlock(system.String(), map[string]string{}))
	}
...
func applyCloaking(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, payload []byte, model string, apiKey string) ([]byte, error) {
	...
	var errUserID error
	payload, errUserID = injectClaudeUserID(
		ctx,
		payload,
		cfg,
		auth,
